...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(enableSprint)
        {
            if(isSprinting)
            {
                ...
                currentSpeedModifier = isCrouched ? speedReduction : 1f;
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            // Apply the current speed modifier based on sprinting or crouching state.
            targetVelocity *= currentSpeedModifier * (isSprinting ? sprintSpeed : walkSpeed);
    ...
    private void Crouch()
    {
        // Toggle the isCrouched state
        isCrouched = !isCrouched;

        if (isCrouched)
        {
            transform.localScale = new Vector3(originalScale.x, crouchHeight, originalScale.z);
            currentSpeedModifier = speedReduction;
        }
        else
        {
